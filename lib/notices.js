...
export async function extractNoticeFromText({ text, source_url, posted_by, posted_at }, { openaiKey, chatModel } = {}) {
  ...
  // Fetch existing active topic_keys so the extractor REUSES a key when the
  // notice concerns the same subject (that is what makes supersession fire).
  let existingKeys = [];
  try {
    const sbx = supabaseAdmin();
    const { data } = await sbx.from('notices').select('topic_key,title,product,model').eq('status', 'active').limit(1000);
    const seen = new Set();
    for (const r of (data || [])) {
      if (r.topic_key && !seen.has(r.topic_key)) { seen.add(r.topic_key); existingKeys.push(`${r.topic_key} - ${r.title || ''} (${r.product}/${r.model})`); }
    }
  } catch (e) { /* ignore */ }
  const sys = 'You convert an internal FundedNext CEx notice into structured JSON for a support knowledge base. ' +
    ...
    'CONDITIONAL POLICIES: if the notice sets different rules by account size, purchase date, region, or model, keep them as ONE entry whose answer_text spells out every branch - do NOT split a single conditional policy into separate entries. ' +
    (existingKeys.length ? ('REUSE EXISTING KEYS: below are topic_keys already in the knowledge base. If this notice concerns the SAME subject as one of them, you MUST reuse that EXACT topic_key and set change_type to "replace" so it supersedes the older version; only invent a new key for a genuinely new subject.\nEXISTING KEYS:\n' + existingKeys.slice(0, 250).join('\n')) : '');
