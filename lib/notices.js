  ...
  const scored = res.data
    ...
    .filter((r) => {
      ...
      if (model !== 'all') {
        ...
        if (namedModels.length && !namedModels.some((item) => item.slug === model)) return false;
        const normalizedNotice = noticeText.toLowerCase().replace(/[–—]/g, '-');
        if (model === 'stellar-1-step' && /\b(?:stellar\s+)?(?:2-step|2 step|two-step|two step)\b/.test(normalizedNotice) && !/\bstellar\s+(?:1-step|1 step|one-step|one step|phase 1|phase one)\b/.test(normalizedNotice)) return false;
        if (model === 'stellar-2-step' && /\b(?:stellar\s+)?(?:1-step|1 step|one-step|one step)\b/.test(normalizedNotice) && !/\bstellar\s+(?:2-step|2 step|two-step|two step|phase 2|phase two)\b/.test(normalizedNotice)) return false;
      }
      return true;
    });
