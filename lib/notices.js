...
// ============================================================================
import { supabaseAdmin, getKeys, openaiEmbed, openaiChatDetailed, modelsMentioned } from './server';
...
export async function extractNoticeFromText({ text, source_url, posted_by, posted_at }, { openaiKey, chatModel } = {}) {
  ...
  // Extraction always uses a small OpenAI model. The workspace answer model
  // may be a Groq-only ID, which OpenAI correctly rejects.
  const completion = await openaiChatDetailed(openaiKey, 'gpt-4o-mini', [
...
export async function retrieveNotices(sb, { openaiKey, question, product = 'both', model = 'all', limit = 6 }) {
  ...
  const res = await sb.rpc('match_notice_chunks', { query_embedding: vec, match_threshold: 0.25, match_count: 20 });
  ...
  const rejected = [];
  const noticeStopWords = new Set(['about','account','accounts','after','again','also','another','client','could','fundednext','have','know','notice','please','says','stellar','their','there','these','they','this','trader','trading','want','wants','what','when','where','which','with','would']);
  const meaningfulTerms = [...new Set(String(question || '').toLowerCase().match(/[a-z0-9]+/g) || [])]
    .filter((term) => term.length >= 4 && !noticeStopWords.has(term));
  const questionText = String(question || '').toLowerCase();
  const payoutIntent = /\b(?:performance reward|payout|withdraw|withdrawal|cash ?out|reward request)\b/.test(questionText);
  const payInIntent = /\b(?:purchase|buy|checkout|registration fee|pay for|payment for|deposit method|account price)\b/.test(questionText);
  // A calendar or duration update is not evidence of an eligibility rule. This
  // was particularly dangerous for Competition questions: the 14-day Notice
  // is genuine, but it says nothing about the separate five-trades-on-five-
  // days requirement. Keep this semantic guard generic so other "minimum
  // trading days" questions cannot be answered by an unrelated schedule.
  const asksMinimumTradingRequirement = /\bminimum\b.{0,45}\b(?:trades?|trading days?)\b|\b(?:trades?|trading days?)\b.{0,45}\bminimum\b|\b(?:how many|number of)\s+days?\b.{0,35}\b(?:must|need|have to|do i)\b.{0,35}\btrade\b/.test(questionText);
  const scored = res.data
    .map((r) => ({ ...r, meta: metaById[r.article_id] }))
    .filter((r) => r.meta) // active only
    .filter((r) => {
      const searchableNotice = `${r.meta.title || ''}\n${r.meta.answer_text || ''}\n${r.meta.raw_excerpt || ''}`.toLowerCase();
      if (asksMinimumTradingRequirement && !(/\bminimum\b/.test(searchableNotice) && /\b(?:trades?|trading days?)\b/.test(searchableNotice))) {
        rejected.push({ id: r.article_id, title: r.meta.title, url: r.meta.source_url, kind: 'notice', reason: 'Notice does not state the requested minimum trading requirement', similarity: Number(r.similarity || 0) });
        return false;
      }
      if (payoutIntent && !payInIntent && /payments?\s*\(pay-?in\)|payin/i.test(`${r.meta.category || ''} ${r.meta.topic_key || ''}`)) {
        rejected.push({ id: r.article_id, title: r.meta.title, url: r.meta.source_url, kind: 'notice', reason: 'Pay-in Notice excluded from a Performance Reward/withdrawal question', similarity: Number(r.similarity || 0) });
        return false;
      }
      const p = r.meta.product || 'both';
      if (product !== 'both' && p !== 'both' && p !== product) {
        rejected.push({ id: r.article_id, title: r.meta.title, url: r.meta.source_url, kind: 'notice', reason: `Wrong product: ${p} notice for ${product}`, similarity: Number(r.similarity || 0) });
        return false;
      }
      const m = r.meta.model || 'all';
      const noticeText = `${r.meta.title || ''}\n${r.meta.answer_text || ''}\n${r.meta.raw_excerpt || ''}`;
      const namedModels = modelsMentioned(noticeText);
      // "All models" means the customer has not chosen a model, not that a
      // model-specific CEx notice may be treated as universal. These notices
      // must never be injected as answer context until a model is selected or
      // the customer explicitly requests a comparison.
      if (model === 'all' && (m !== 'all' || namedModels.length)) {
        rejected.push({ id: r.article_id, title: r.meta.title, url: r.meta.source_url, kind: 'notice', reason: `Model-specific notice excluded from an All-model answer (${m !== 'all' ? m : namedModels.map((item) => item.slug).join(', ')})`, similarity: Number(r.similarity || 0) });
        return false;
      }
      if (model !== 'all' && m !== 'all' && m !== model) {
        rejected.push({ id: r.article_id, title: r.meta.title, url: r.meta.source_url, kind: 'notice', reason: `Wrong model: ${m} notice for ${model}`, similarity: Number(r.similarity || 0) });
        return false;
      }
      // A notice marked "all" can still contain model-specific wording. Never
      // allow an explicitly different model into a scoped answer, even when the
      // stored metadata was accidentally too broad.
      if (model !== 'all') {
        if (namedModels.length && !namedModels.some((item) => item.slug === model)) {
          rejected.push({ id: r.article_id, title: r.meta.title, url: r.meta.source_url, kind: 'notice', reason: `Notice text names ${namedModels.map((item) => item.slug).join(', ')}; question requires ${model}`, similarity: Number(r.similarity || 0) });
          return false;
        }
        const normalizedNotice = noticeText.toLowerCase().replace(/[–—]/g, '-');
        if (model === 'stellar-1-step' && /\b(?:stellar\s+)?(?:2-step|2 step|two-step|two step)\b/.test(normalizedNotice) && !/\bstellar\s+(?:1-step|1 step|one-step|one step|phase 1|phase one)\b/.test(normalizedNotice)) {
          rejected.push({ id: r.article_id, title: r.meta.title, url: r.meta.source_url, kind: 'notice', reason: 'Notice says 2-Step; question requires Stellar 1-Step', similarity: Number(r.similarity || 0) });
          return false;
        }
        if (model === 'stellar-2-step' && /\b(?:stellar\s+)?(?:1-step|1 step|one-step|one step)\b/.test(normalizedNotice) && !/\bstellar\s+(?:2-step|2 step|two-step|two step|phase 2|phase two)\b/.test(normalizedNotice)) {
          rejected.push({ id: r.article_id, title: r.meta.title, url: r.meta.source_url, kind: 'notice', reason: 'Notice says 1-Step; question requires Stellar 2-Step', similarity: Number(r.similarity || 0) });
          return false;
        }
      }
      const searchable = `${r.meta.title || ''} ${r.meta.answer_text || ''} ${r.meta.keywords || ''} ${r.content || ''}`.toLowerCase();
      const overlap = meaningfulTerms.filter((term) => searchable.includes(term));
      const similarity = Number(r.similarity || 0);
      const requiredOverlap = meaningfulTerms.length >= 6 ? 2 : 1;
      // Authority applies only after relevance. Merely naming the Account is
      // not enough for a notice to enter the answer context.
      if (similarity < 0.55 && overlap.length < requiredOverlap) {
        rejected.push({ id: r.article_id, title: r.meta.title, url: r.meta.source_url, kind: 'notice', reason: `Low topical relevance (${(similarity * 100).toFixed(1)}% similarity; ${overlap.length} meaningful query term${overlap.length === 1 ? '' : 's'} matched)`, similarity });
        return false;
      }
      return true;
    });

  const topicWinners = new Map();
  for (const item of scored) {
    // One ClickUp message can be stored both as a parent Notice and as one or
    // more extracted entries. Keep one result from that source message without
    // collapsing separate Notices that intentionally share a broad topic key
    // (for example driving-licence updates for different countries).
    const topic = String(item.meta?.parent_entry_id || item.meta?.clickup_message_id || item.article_id || '').split('__')[0];
    const previous = topicWinners.get(topic);
    const itemDate = Date.parse(item.meta?.posted_at || 0) || 0;
    const previousDate = Date.parse(previous?.meta?.posted_at || 0) || 0;
    if (!previous || itemDate > previousDate) topicWinners.set(topic, item);
  }
  const deduped = [...topicWinners.values()].sort((a, b) => Number(b.similarity || 0) - Number(a.similarity || 0));
  const seen = new Set();
  const matches = [];
  for (const r of deduped) {
    ...
    matches.push(r);
    if (matches.length >= Math.min(limit, 3)) break;
  }
  ...
  return { matches, escalations, rejected: rejected.slice(0, 20) };
}
...
// Embed a single active notice into notice_chunks (fast; used by reactivate/save).
export async function embedOneNotice(entry_id, sb = supabaseAdmin(), { openaiKey } = {}) {
  if (!openaiKey) { const k = await getKeys(); openaiKey = k.openaiKey; }
  if (!openaiKey) throw new Error('OpenAI key is not set.');
  const { data } = await sb.from('notices').select('*').eq('entry_id', entry_id).maybeSingle();
  await sb.from('notice_chunks').delete().eq('article_id', entry_id);
  if (!data || data.status !== 'active' || data.review_flag) return { indexed: 0 };
  const text = `${data.title || ''}\n${data.answer_text || ''}\n${(data.keywords || []).join(' ')}`.trim();
  const [vec] = await openaiEmbed(openaiKey, [text]);
  await sb.from('notice_chunks').insert([{
    article_id: data.entry_id, article_title: data.title || '(notice)', article_url: data.source_url || '',
    chunk_index: 0, content: `${data.title || ''}. ${data.answer_text || ''}`.trim(), embedding: vec
  }]);
  return { indexed: 1 };
}

// Fast status change: NO full re-index. Activating embeds just this one notice;
// expiring/superseding drops just its chunks. Returns quickly.
export async function applyNoticeStatusFast(entry_id, status, sb = supabaseAdmin(), { openaiKey } = {}) {
  await setNoticeStatus(entry_id, status, sb);
  if (status === 'active') await embedOneNotice(entry_id, sb, { openaiKey });
  else await sb.from('notice_chunks').delete().eq('article_id', entry_id);
  try { await sb.from('settings').upsert({ key: 'notices_indexed_at', value: new Date().toISOString() }); } catch (e) { /* ignore */ }
  return { ok: true };
}

// Permanently remove a notice and its embedding.
export async function deleteNotice(entry_id, sb = supabaseAdmin()) {
  await sb.from('notice_chunks').delete().eq('article_id', entry_id);
  const { error } = await sb.from('notices').delete().eq('entry_id', entry_id);
  if (error) throw new Error(error.message);
  try { await sb.from('settings').upsert({ key: 'notices_indexed_at', value: new Date().toISOString() }); } catch (e) { /* ignore */ }
  return { ok: true };
}
