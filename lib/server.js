...
export async function verifyGrounding({ question, answer, context, provider, model, openaiKey, groqKey }) {
  ...
  if (!key) return null;
  const sys =
    'You are a strict grounding checker for a FundedNext FAQ assistant. You are given FAQ EVIDENCE and a draft ANSWER. ' +
    'Decide whether EVERY factual claim in the answer is directly and explicitly stated in the evidence. Be strict: a specific ' +
    'rule, number, permission, or prohibition that is not clearly present in the evidence is NOT grounded, even if it sounds ' +
    'plausible or is common knowledge. Outside/general knowledge does not count as evidence. ' +
    'Return STRICT JSON only: {"grounded": true|false, "score": 0-100, "unsupported": ["short claim", ...]}. ' +
    'score = how well the evidence supports the whole answer (100 = every claim explicitly supported, 0 = not supported).';
  try {
    const { content } = await openaiChatDetailed(key, model, [
      { role: 'system', content: sys },
