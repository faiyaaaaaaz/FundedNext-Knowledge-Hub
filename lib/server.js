...
const SCOPE_CATALOG_VERSION = 8;

function scopeText(value) {
  return normalizedArticleValue(value).toLowerCase().replace(/[–—]/g, '-');
}
...
export async function getScopeCatalog(sb = supabaseAdmin(), verifiedArticleOverrides = null) {
  ...
  const catalogue = BASE_KNOWLEDGE_MODELS.map((model) => ({ ...model, articleCount: 0, articles: [] }));
  for (const article of articles) {
    const text = `${article.title || ''}\n${article.body || ''}`;
    const override = articleOverrides[String(article.intercom_id)];
    const mentionedModels = override
      ? (override.model !== 'all' ? catalogue.filter((model) => model.slug === override.model && model.product === override.product) : [])
      : modelsMentioned(text, catalogue);
    for (const model of mentionedModels) {
      model.articleCount++;
      if (model.articles.length < 80 && article.title && !model.articles.some((item) => item.id === String(article.intercom_id))) {
        model.articles.push({ id: String(article.intercom_id), title: article.title, url: article.url || '', scope: override || null });
      }
    }
  }

  // Future-proof discovery: a genuinely new model normally appears in several
  // FAQ titles. Requiring two separate articles prevents generic phrases from
  // becoming customer-facing selector options. Single-title candidates remain
  // visible to Admins as review items.
  const candidates = new Map();
  const ignored = /^(fundednext|futures fundednext|fundednext futures|available|challenge|fundednext challenge|fundednext account|trading|live|demo|free trial|account|customer|client)$/i;
  const policyPhrase = /\b(?:available|reset|account sizes?|mll|dll|payout|performance reward|allocation|kyc|verification|merge|merging|scale|scaling|restricted|strategy|sub-affiliation|profit|loss|drawdown|leverage|consistency|trading day|benchmark)\b/i;
  for (const article of articles) {
    const title = normalizedArticleValue(article.title);
    const url = String(article.url || '');
    // Only English canonical titles count toward automatic publication. This
    // prevents translated copies of one FAQ from falsely satisfying the
    // repeated-evidence requirement.
    const englishCanonical = /\/en(?:\/|$)/i.test(url);
    if (!englishCanonical) continue;
    const product = /helpfutures\.fundednext\.com/i.test(url) || /\bfundednext futures?\b/i.test(title)
      ? 'futures'
      : /help\.fundednext\.com/i.test(url) ? 'cfd' : null;
    if (!product) continue;
    const patterns = [
      /\b([A-Z][A-Za-z0-9:-]*(?:\s+(?:[A-Z0-9][A-Za-z0-9:-]*|1-Step|2-Step)){0,4})\s+(?:Account|Challenge|Model)\b/g,
      /\b(Model\s+[A-Z0-9:-]{3,})\b/g
    ];
    for (const pattern of patterns) {
      for (const match of title.matchAll(pattern)) {
        const name = match[1].trim();
        const normalizedName = name.toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim();
        const alreadyKnown = catalogue.some((model) =>
          model.name.toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim() === normalizedName ||
          model.aliases.some((alias) => alias.toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim() === normalizedName)
        );
        if (ignored.test(name) || policyPhrase.test(name) || alreadyKnown) continue;
        const key = `${product}:${name.toLowerCase()}`;
        const candidate = candidates.get(key) || { name, product, articles: new Map() };
        candidate.articles.set(String(article.intercom_id), { id: String(article.intercom_id), title, url }); candidates.set(key, candidate);
      }
    }
  }
  for (const candidate of candidates.values()) {
    const evidence = [...candidate.articles.values()];
    const articleCount = evidence.length;
    catalogue.push({
      slug: `${candidate.product}-${candidate.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')}`,
      product: candidate.product, name: candidate.name,
      // Discovery is advisory only. A repeated phrase is not enough to make it
      // a live Account model because policy words can occur in many FAQ titles.
      // Admin must explicitly promote a candidate from review.
      status: 'review',
      aliases: [candidate.name.toLowerCase()], articleCount,
      articles: evidence.slice(0, 80), autoDetected: true
    });
  }
  for (const candidate of discoverModels(articles, catalogue)) catalogue.push(candidate);
  let overrides = {};
  try {
    const raw = await getRaw(sb, 'scope_model_overrides');
    overrides = raw ? (typeof raw === 'string' ? JSON.parse(raw) : raw) : {};
  } catch { overrides = {}; }
  // Keep reviewed models even when later article edits remove the original
  // extraction pattern. Explicit aliases remain available to retrieval.
  for(const [slug,override] of Object.entries(overrides)) {
    if(!catalogue.some(m=>m.slug===slug)&&override.name&&override.product) catalogue.push({slug,name:override.name,product:override.product,status:'review',aliases:override.aliases||[override.name.toLowerCase()],articles:override.articles||[],articleCount:0});
  }
  const governed = catalogue.map((model) => {
    const override = overrides[model.slug];
    if (!override) return model;
    return {
      ...model,
      name: String(override.name || model.name),
      product: ['cfd', 'futures'].includes(override.product) ? override.product : model.product,
      status: ['current', 'previous', 'review','rejected'].includes(override.status) ? override.status : model.status,
      aliases: Array.isArray(override.aliases) && override.aliases.length ? override.aliases : model.aliases,
      adminConfirmed: true
    };
  });
  return {
    ...
    models: governed.sort((a, b) => {
      const statusRank = { current: 0, previous: 1, review: 2 };
      return a.product.localeCompare(b.product) || (statusRank[a.status] - statusRank[b.status]) || a.name.localeCompare(b.name);
    })
  };
}
