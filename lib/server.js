...
function signSession({ role, version, sessionId, name, email, authProvider }) {
  const payload = b64url(JSON.stringify({
    role, version, sessionId, name: name || null, email: email || null,
...
export async function createLoginSession(password) {
  ...
  return {
    token: signSession({ role, version, sessionId, name, authProvider: 'password' }),
...
export async function createGoogleLoginSession(accessToken) {
  ...
  if (!configured.length) throw new Error('Google access has not been configured by an Admin yet.');
  const domain = user.email.split('@')[1]?.toLowerCase();
  if (!domain || !configured.includes(domain)) {
  ...
  const token = signSession({
    role: 'agent', version, sessionId, name, email: user.email, authProvider: 'google'
  });
  return {
    token, role: 'agent', sessionId, name, email: user.email,
...
export async function saveKeys({ intercomToken, openaiKey, groqKey, chatModel, chatProvider, chatPrompt, allowedGoogleDomains }) {
  const sb = supabaseAdmin();
  const rows = [];
