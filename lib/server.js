...
export async function createGoogleLoginSession(accessToken) {
  ...
  if (!admins.length) throw new Error('ADMIN_GOOGLE_EMAILS is not configured in Vercel.');
...
export async function clarifyQuery({ question, provider, model, openaiKey, groqKey }) {
  ...
  const sys =
    ...
    'Choices must be self-explanatory and non-overlapping. Format each as "Choice — what this choice means" so a new Agent can select confidently. ' +
