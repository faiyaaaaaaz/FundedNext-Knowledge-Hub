...
export async function getScopeCatalog(sb = supabaseAdmin(), verifiedArticleOverrides = null) {
  const { data, error } = await sb.from('articles').select('intercom_id,title,body,url').eq('state', 'published').limit(5000);
  if (error) throw new Error('Could not build the Account catalogue: ' + error.message);
  const articles = data || [];
...
export async function syncStep(sb, { intercomToken, openaiKey }) {
  ...
  const { data: stored, error: e1 } = await sb.from('articles').select('intercom_id,title,body,url,content_hash');
...
export async function knowledgeStatus(sb = supabaseAdmin()) {
  const [docs, articleOverrides, articleRows] = await Promise.all([
    ...
    sb.from('articles').select('intercom_id,title,url,body').eq('state', 'published').order('title').limit(5000)
  ]);
  if (articleRows.error) throw new Error('Could not load article scopes: ' + articleRows.error.message);
  ...
  const articleScopes = (articleRows.data || []).map((article) => {
    const override = articleOverrides[String(article.intercom_id)];
