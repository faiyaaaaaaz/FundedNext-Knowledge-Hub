    ...
    try {
      ...
      if (identity.authProvider === 'master_password' && data.role === 'admin') {
        ...
        const [currentVersion, configuredHash] = await Promise.all([
          getRaw(sb, MASTER_SESSION_VERSION_KEY), getRaw(sb, MASTER_PASSWORD_KEY)
        ]);
...
export async function logActivity(entry) {
  try {
    const { data } = await supabaseAdmin().from('activity_logs').insert({
...
async function fetchAllHelpCenterGroups(token, kind) {
  ...
  do {
    ...
    const response = await fetchWithTimeout(url.toString(), { headers: {
      Authorization: `Bearer ${token}`, Accept: 'application/json', 'Intercom-Version': '2.11'
    } }, 15000, `Intercom ${kind} took too long to respond`);
    if (!response.ok) throw new Error(`Intercom ${kind} returned ${response.status}. ${(await response.text()).slice(0, 200)}`);
...
export async function syncStep(sb, { intercomToken, openaiKey }) {
  ...
  const [articles, collections, sections] = await Promise.all([
    fetchAllPublishedArticles(intercomToken),
    fetchAllHelpCenterGroups(intercomToken, 'collections'),
    fetchAllHelpCenterGroups(intercomToken, 'sections')
  ]);
  const articleRegionScopes = buildArticleRegionScopes(articles, collections, sections);
