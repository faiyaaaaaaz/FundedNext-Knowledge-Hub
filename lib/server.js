...
function signSession({ role, version, gv, sessionId, name, email, authProvider, userId }) {
  const payload = b64url(JSON.stringify({
    ...
    authProvider: authProvider || 'google', userId: userId || null,
...
export function modelsMentioned(text, catalog = BASE_KNOWLEDGE_MODELS) {
  ...
  return catalog.filter((model) => model.status !== 'review' && model.aliases.some((alias) => {
...
export async function getScopeCatalog(sb = supabaseAdmin(), verifiedArticleOverrides = null) {
  ...
  const governed = catalogue.map((model) => {
    ...
    return {
      ...
      status: ['current', 'previous', 'review'].includes(override.status) ? override.status : model.status,
...
export async function saveScopeModelOverride(slug, changes, sb = supabaseAdmin()) {
  ...
  overrides[safeSlug] = {
    ...(overrides[safeSlug] || {}),
    ...(['current', 'previous', 'review'].includes(changes?.status) ? { status: changes.status } : {}),
...
export async function getPublishedScopeCatalog(sb = supabaseAdmin()) {
  ...
  const catalog = await getScopeCatalog(sb);
  try { await sb.from('settings').upsert({ key: 'published_scope_catalog', value: JSON.stringify(catalog) }); } catch { /* read still succeeds */ }
  return catalog;
}
