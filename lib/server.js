...
export function modelsMentioned(text, catalog = BASE_KNOWLEDGE_MODELS) {
  ...
  return catalog.filter((model) => model.aliases.some((alias) => haystack.includes(alias.toLowerCase())));
}
...
export async function getScopeCatalog(sb = supabaseAdmin(), verifiedArticleOverrides = null) {
  ...
  for (const candidate of candidates.values()) {
    ...
    catalogue.push({
      ...
      status: articleCount >= 2 ? 'current' : 'review',
