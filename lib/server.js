...
export async function createLoginSession(password) {
  ...
  const payload = b64url(JSON.stringify({ role, version, sessionId, exp: Date.now() + 30 * 24 * 60 * 60 * 1000 }));
  const signature = crypto.createHmac('sha256', encKey()).update(payload).digest('base64url');
  return { token: `${payload}.${signature}`, role, sessionId };
}
...
export async function authenticateRequest(req) {
  ...
  if (token) {
    try {
      ...
      if (data.role === 'admin') return { role: 'admin', sessionId: data.sessionId || 'admin-legacy' };
      if (data.role === 'agent') {
        const current = await getAgentVersion(supabaseAdmin());
        if (String(data.version) === String(current)) return { role: 'agent', sessionId: data.sessionId || 'agent-legacy' };
      }
    } catch { return null; }
  }
  ...
  if (checkPassword(password)) return { role: 'admin', sessionId: 'admin-legacy' };
  return null;
}
