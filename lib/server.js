...
export async function getScopeCatalog(sb = supabaseAdmin()) {
  ...
  for (const article of articles) {
    ...
    for (const model of modelsMentioned(text, catalogue)) {
      model.articleCount++;
      if (model.articles.length < 80 && article.title && !model.articles.some((item) => item.id === String(article.intercom_id))) {
        model.articles.push({ id: String(article.intercom_id), title: article.title, url: article.url || '' });
      }
    }
  }
  ...
  const ignored = /^(fundednext|challenge|fundednext challenge|fundednext account|trading|live|demo|free trial|account|customer|client)$/i;
  const policyPhrase = /\b(?:mll|dll|payout|performance reward|allocation|kyc|verification|merge|merging|scale|scaling|restricted|strategy|sub-affiliation|profit|loss|drawdown|leverage|consistency|trading day|benchmark)\b/i;
  for (const article of articles) {
...
export async function knowledgeStatus(sb = supabaseAdmin()) {
  const [docs, scopeCatalog] = await Promise.all([listKnowledge(sb), getPublishedScopeCatalog(sb)]);
  let indexedChunks = 0;
  try {
  ...
  return { docs, indexedChunks, indexedAt, scopeCatalog };
}
