...
import {discoverModels,applyModelOverrides} from './modelDiscovery';
...
// Session checks happen on every API request. A very short, promise-deduplicated
// cache avoids repeated settings reads when the same warm route handles a GET
// followed immediately by its acknowledgement POST. Security changes still
// propagate quickly, and mutation paths invalidate their local cache at once.
const AUTH_SETTING_CACHE_MS = 15 * 1000;
const authSettingCache = new Map();
async function getAuthSetting(sb, key) {
  const now = Date.now(), cached = authSettingCache.get(key);
  if (cached?.value !== undefined && cached.expiresAt > now) return cached.value;
  if (cached?.promise) return cached.promise;
  const promise = getRaw(sb, key).then((value) => {
    authSettingCache.set(key, { value, expiresAt: Date.now() + AUTH_SETTING_CACHE_MS });
    return value;
  }).catch((error) => { authSettingCache.delete(key); throw error; });
  authSettingCache.set(key, { promise, expiresAt: now + AUTH_SETTING_CACHE_MS });
  return promise;
}
function invalidateAuthSettings(...keys) { for (const key of keys) authSettingCache.delete(key); }

async function getAgentVersion(sb) {
  return (await getAuthSetting(sb, 'agent_session_version')) || '1';
}
...
async function getGlobalVersion(sb) {
  return (await getAuthSetting(sb, 'global_session_version')) || '1';
}

const ALLOWED_GOOGLE_DOMAIN = 'nextventures.io';
const WORKSPACE_AUTH_RELEASE = 'workspace-2026-09-v2';
const MASTER_PASSWORD_KEY = 'master_password_hash_v1';
const MASTER_SESSION_VERSION_KEY = 'master_session_version';
const MASTER_SESSION_MS = 8 * 60 * 60 * 1000;
const MASTER_LOGIN_WINDOW_MS = 15 * 60 * 1000;
const MASTER_LOGIN_MAX_PER_IP = 5;
// Password-backed access has no Google profile, but the rest of the workspace
// needs a stable, non-null principal for tours, history, inbox, and audit rows.
// This address is an internal identifier only; it is never used for email.
export const MASTER_PRINCIPAL = 'master-admin@workspace.local';
...
function signSession({ role, version, gv, sessionId, name, email, authProvider, userId }) {
  const passwordSession = authProvider === 'master_password';
  const payload = b64url(JSON.stringify({
    role, version, gv: gv || '1', sessionId, name: name || null, email: email || null,
    authProvider: authProvider || 'google', userId: userId || null, workspaceRelease: WORKSPACE_AUTH_RELEASE,
    // Password-backed Master sessions are deliberately much shorter than
    // Google sessions and are revocable independently.
    exp: Date.now() + (passwordSession ? MASTER_SESSION_MS : 5 * 24 * 60 * 60 * 1000)
  }));
...
export async function createGoogleLoginSession(accessToken) {
  ...
  if (!user.last_sign_in_at || Date.now()-Date.parse(user.last_sign_in_at)>10*60*1000) throw new Error('Please complete a fresh Google sign-in to start a new workspace session.');
  const normalizedEmail = user.email.toLowerCase();
  const domain = user.email.split('@')[1]?.toLowerCase();
  if (domain !== ALLOWED_GOOGLE_DOMAIN) throw new Error('Only nextventures.io Google accounts can access this workspace.');
  const admins = adminEmails();

  const version = await getAgentVersion(sb);
  const gv = await getGlobalVersion(sb);
  const role = admins.includes(normalizedEmail) ? 'admin' : 'agent';
...
function masterPasswordHash(password, salt = crypto.randomBytes(16).toString('hex')) {
  const N = 32768, r = 8, p = 3;
  // The server-only encryption secret acts as a pepper, so a database-only
  // leak is not sufficient for offline password guessing.
  const material = crypto.createHmac('sha256', encKey()).update(String(password)).digest();
  const hash = crypto.scryptSync(material, salt, 64, { N, r, p, maxmem: 64 * 1024 * 1024 }).toString('hex');
  return `scrypt-v1$${N}$${r}$${p}$${salt}$${hash}`;
}

function verifyMasterPassword(password, stored) {
  try {
    const [version, n, r, p, salt, expected] = String(stored || '').split('$');
    if (version !== 'scrypt-v1' || !salt || !expected) return false;
    const material = crypto.createHmac('sha256', encKey()).update(String(password)).digest();
    const actual = crypto.scryptSync(material, salt, 64, {
...
function masterIpHash(req) {
  const realIp = String(req?.headers?.['x-real-ip'] || '').trim();
  const forwarded = String(req?.headers?.['x-forwarded-for'] || '').split(',')[0].trim();
  const ip = realIp || forwarded || String(req?.socket?.remoteAddress || 'unknown');
  return crypto.createHmac('sha256', encKey()).update(ip).digest('hex');
}
...
async function enforceMasterLoginRateLimit(sb, req) {
  const ipHash = masterIpHash(req);
  const since = new Date(Date.now() - MASTER_LOGIN_WINDOW_MS).toISOString();
  const { count, error } = await sb.from('activity_logs').select('id', { count: 'exact', head: true })
    .eq('event_type', 'master_login_failed').gte('created_at', since).contains('metadata', { ipHash });
  if (error) throw new Error('Master access security check is unavailable. Please try again shortly.');
  if (Number(count || 0) >= MASTER_LOGIN_MAX_PER_IP) {
    const error = new Error('Too many unsuccessful attempts. Wait 15 minutes before trying again.');
...
export async function createMasterLoginSession(password, req) {
  const clean = String(password || '');
  const sb = supabaseAdmin();
  const ipHash = await enforceMasterLoginRateLimit(sb, req);
  const stored = await getRaw(sb, MASTER_PASSWORD_KEY);
  const valid = clean.length <= 256 && !!stored && verifyMasterPassword(clean, stored);
  if (!valid) {
    await sb.from('activity_logs').insert({
      actor_role: null, session_id: null, user_name: null, user_email: null,
      auth_provider: 'master_password', event_type: 'master_login_failed', success: false,
      metadata: { ipHash, userAgent: String(req?.headers?.['user-agent'] || '').slice(0, 300) }
    });
    return null;
  }
  const version = (await getRaw(sb, MASTER_SESSION_VERSION_KEY)) || '1';
  const gv = await getGlobalVersion(sb);
  const sessionId = crypto.randomBytes(16).toString('hex');
  const token = signSession({
    role: 'admin', version, gv, sessionId, name: 'Master Admin', email: MASTER_PRINCIPAL,
    authProvider: 'master_password', userId: null
  });
  // Keep the internal principal server-side; the UI should continue to show
  // "Master Admin" without presenting it as a real email address.
  return { token, role: 'admin', sessionId, name: 'Master Admin', email: '', authProvider: 'master_password', expiresIn: MASTER_SESSION_MS / 1000 };
}

export async function authenticateRequest(req) {
  const headerToken = req.headers['x-app-session'];
  // A real Google token explicitly wins over any stale password cookie. The
  // browser uses only this non-secret marker for HttpOnly Master sessions.
  const token = headerToken && headerToken !== 'master-cookie'
    ? headerToken
    : (req.cookies?.['__Host-fn_master_session'] || req.cookies?.fn_master_session);
  if (token) {
    try {
      ...
      if (!data.exp || Date.now() > data.exp || data.workspaceRelease !== WORKSPACE_AUTH_RELEASE) return null;
      ...
      const identity = {
        ...
        // Upgrade already-issued Master cookies that contain a null email. This
        // keeps the hotfix effective without asking the user to sign out first.
        email: data.email || (data.authProvider === 'master_password' ? MASTER_PRINCIPAL : null),
        authProvider: data.authProvider || 'google',
        userId: data.userId || null
      };
      if (identity.authProvider === 'master_password' && data.role === 'admin') {
        if (!hasSameOrigin(req)) return null;
        const sb = supabaseAdmin();
        const [currentVersion, configuredHash] = await Promise.all([
          getAuthSetting(sb, MASTER_SESSION_VERSION_KEY), getAuthSetting(sb, MASTER_PASSWORD_KEY)
        ]);
        if (configuredHash && String(data.version || '1') === String(currentVersion || '1')) return identity;
        return null;
      }
      // Re-check both domain and current Admin allowlist on every Google-backed
      // request so removing an email removes access without a code change.
      if (identity.authProvider !== 'google' || !identity.email) return null;
      if (identity.email.split('@')[1]?.toLowerCase() !== ALLOWED_GOOGLE_DOMAIN) return null;
      if (data.role === 'admin') {
        if (!adminEmails().includes(identity.email.toLowerCase())) return null;
        return identity;
      }
...
export async function logActivity(entry, options = {}) {
  try {
    const sb = options.sb || supabaseAdmin();
    const insert = sb.from('activity_logs').insert({
    ...
    // Most audit callers only need confirmation that the write succeeded.
    // Avoid asking PostgREST to read the new row back in those hot paths.
...
export async function getAgentAccessStatus() {
  const masterPasswordSet = !!(await getRaw(supabaseAdmin(), MASTER_PASSWORD_KEY));
  return {
    agentPasswordSet: false,
    agentLoginMode: 'google_only',
    masterPasswordSet,
    masterLoginMode: masterPasswordSet ? 'password_or_google' : 'google_only',
    allowedGoogleDomain: ALLOWED_GOOGLE_DOMAIN,
    adminGoogleConfigured: adminEmails().length > 0
  };
}

function validateMasterPassword(password) {
  const clean = String(password || '');
  if (clean.length < 16) throw new Error('Master password must be at least 16 characters. A long unique passphrase is recommended.');
  if (clean.length > 256) throw new Error('Master password cannot exceed 256 characters.');
  if (/^(.)\1+$/.test(clean) || /^(?:password|fundednext|admin|master|qwerty|letmein)\d*[!@#$%^&*]*$/i.test(clean)) {
    throw new Error('Choose a less predictable Master password.');
  }
  return clean;
}

export async function setMasterPassword(password) {
  const clean = validateMasterPassword(password);
  const sb = supabaseAdmin();
  const nextVersion = String(Number((await getRaw(sb, MASTER_SESSION_VERSION_KEY)) || '1') + 1);
  const { error } = await sb.from('settings').upsert([
    { key: MASTER_PASSWORD_KEY, value: masterPasswordHash(clean) },
    { key: MASTER_SESSION_VERSION_KEY, value: nextVersion }
  ]);
  if (error) throw new Error('Could not update Master password access: ' + error.message);
  invalidateAuthSettings(MASTER_PASSWORD_KEY, MASTER_SESSION_VERSION_KEY);
}

export async function disableMasterPassword() {
  const sb = supabaseAdmin();
  const nextVersion = String(Number((await getRaw(sb, MASTER_SESSION_VERSION_KEY)) || '1') + 1);
  const { error: deleteError } = await sb.from('settings').delete().eq('key', MASTER_PASSWORD_KEY);
  if (deleteError) throw new Error('Could not disable Master password access: ' + deleteError.message);
  const { error } = await sb.from('settings').upsert({ key: MASTER_SESSION_VERSION_KEY, value: nextVersion });
  if (error) throw new Error('Could not revoke Master password sessions: ' + error.message);
  invalidateAuthSettings(MASTER_PASSWORD_KEY, MASTER_SESSION_VERSION_KEY);
}
...
export async function revokeAgentSessions() {
  ...
  invalidateAuthSettings('agent_session_version', 'global_session_version');
}
...
export async function getKeys() {
  ...
  const adminFallback = await getRaw(sb, 'admin_auto_fallback');
  const fallbackProvider = await getRaw(sb, 'fallback_provider');
  const fallbackModel = await getRaw(sb, 'fallback_model');
  return {
    ...
    adminAutoFallback: adminFallback == null ? true : adminFallback === 'true',
    fallbackProvider: fallbackProvider === 'groq' ? 'groq' : 'openai',
...
export async function saveKeys({ intercomToken, openaiKey, groqKey, chatModel, chatProvider, chatPrompt, allowedGoogleDomains, smartRetrieval, normalUserGptFallback, adminAutoFallback, fallbackProvider, fallbackModel }) {
  const sb = supabaseAdmin();
  const rows = [];
  ...
  if (typeof adminAutoFallback === 'boolean') rows.push({ key: 'admin_auto_fallback', value: adminAutoFallback ? 'true' : 'false' });
  if (fallbackProvider === 'openai' || fallbackProvider === 'groq') rows.push({ key: 'fallback_provider', value: fallbackProvider });
  if (typeof fallbackModel === 'string' && fallbackModel.trim()) rows.push({ key: 'fallback_model', value: fallbackModel.trim() });
...
export async function getRelevantSnippets(question, limit = 6) {
  ...
  const { data, error } = await sb
    .from('ai_snippets')
    .select('id,title,trigger_terms,instruction,source_dispute_id')
    .eq('active', true)
    .limit(200);
  if (error) {
  ...
  const normalizedQuestion = String(question).toLowerCase().replace(/[–—]/g, '-');
  const words = new Set(normalizedQuestion.match(/[a-z0-9-]{3,}/g) || []);
  const weak = new Set(['account','accounts','fundednext','question','answer','minimum','maximum','rules','rule','trading','performance','reward','challenge','model']);
  const relevant = (data || []).map((snippet) => {
    const triggers = String(snippet.trigger_terms || '').toLowerCase().match(/[a-z0-9-]{3,}/g) || [];
    const meaningful = [...new Set(triggers.filter((word) => !weak.has(word)))];
    const matched = meaningful.filter((word) => words.has(word));
    const phrases = String(snippet.trigger_terms || '').toLowerCase().split(/[,;|]/).map((item) => item.trim()).filter((item) => item.split(/\s+/).length >= 2);
    const exactPhrase = phrases.some((phrase) => normalizedQuestion.includes(phrase));
    const ratio = meaningful.length ? matched.length / meaningful.length : 0;
    const eligible = exactPhrase || (matched.length >= 2 && ratio >= 0.4) || (meaningful.length === 1 && matched.length === 1);
    const score = exactPhrase ? 20 + matched.length : matched.length + ratio;
    return { ...snippet, _score: score, _triggerMatch: { matched, meaningful: meaningful.length, exactPhrase }, _eligible: eligible };
  }).filter((item) => item._eligible).sort((a, b) => b._score - a._score).slice(0, limit)
    .map(({ _eligible, ...item }) => item);

  // A reviewed correction is only as useful as the evidence approved with it.
  // Preserve those captured FAQ/Notice passages at answer time instead of
  // discarding provenance when the snippet row is activated.
  const disputeIds = [...new Set(relevant.map((item) => item.source_dispute_id).filter(Boolean))];
  if (!disputeIds.length) return relevant;
  const { data: disputes, error: disputeError } = await sb.from('disputes')
    .select('id,status,sources').in('id', disputeIds);
  if (disputeError) throw new Error('Could not load correction evidence: ' + disputeError.message);
  const evidenceByDispute = new Map((disputes || []).map((dispute) => [String(dispute.id),
    dispute.status === 'snippet_generated' && Array.isArray(dispute.sources)
      ? dispute.sources.filter((source) => source?.type === 'correction_evidence' && source?.excerpt).slice(0, 4)
      : []
  ]));
  return relevant.map((item) => ({
    ...item,
    _evidence: evidenceByDispute.get(String(item.source_dispute_id)) || []
  }));
}

// Canonical product/model catalogue. Known historical products deliberately
// remain here because Agents still receive questions about them. New names are
// added from repeated FAQ-title evidence by getScopeCatalog below.
export const BASE_KNOWLEDGE_MODELS = [
  { slug: 'stellar-1-step', product: 'cfd', name: 'Stellar 1-Step', status: 'current', aliases: ['stellar 1-step', 'stellar 1 step', 'stellar one-step', 'stellar one step', 'stellar phase 1', 'stellar phase one'] },
  { slug: 'stellar-2-step', product: 'cfd', name: 'Stellar 2-Step', status: 'current', aliases: ['stellar 2-step', 'stellar two-step'] },
  { slug: 'stellar-lite', product: 'cfd', name: 'Stellar Lite', status: 'current', aliases: ['stellar lite'] },
  { slug: 'stellar-instant', product: 'cfd', name: 'Stellar Instant', status: 'current', aliases: ['stellar instant'] },
  { slug: 'competition', product: 'cfd', name: 'Competition', status: 'current', aliases: ['fundednext competition', "fundednext traders' competition", 'fundednext traders competition', 'traders competition', 'trading competition', 'competition account'] },
  { slug: 'no-dll-fnl-001', product: 'cfd', name: 'No DLL 1-Step CFD — FNL:001', status: 'current', aliases: ['no dll 1-step', 'no dll 1 step', 'fnl:001', 'fnl 001'] },
  { slug: 'fnl-002-2-step-25k', product: 'cfd', name: 'FNL:002 12% MLL 3-Day Payout 2-Step 25K', status: 'previous', aliases: ['fnl:002', 'fnl 002', '12% mll 3-day payout 2-step 25k', '12% mll 3 day payout 2-step 25k'] },
  { slug: 'evaluation', product: 'cfd', name: 'Evaluation', status: 'previous', aliases: ['evaluation fundednext', 'evaluation account', 'evaluation model'] },
  { slug: 'express', product: 'cfd', name: 'Express', status: 'previous', aliases: ['express fundednext', 'express account', 'express consistency', 'express non-consistency'] },
  { slug: 'futures-rapid', product: 'futures', name: 'Futures Rapid', status: 'current', aliases: ['futures rapid', 'rapid challenge', 'rapid fundednext account'] },
  { slug: 'futures-legacy', product: 'futures', name: 'Futures Legacy', status: 'current', aliases: ['futures legacy', 'legacy challenge', 'legacy fundednext account'] },
  { slug: 'futures-bolt', product: 'futures', name: 'Futures Bolt', status: 'current', aliases: ['futures bolt', 'bolt challenge', 'bolt fundednext account'] },
  { slug: 'futures-flex', product: 'futures', name: 'Futures Flex', status: 'current', aliases: ['futures flex', 'flex challenge', 'flex fundednext account'] }
];

const SCOPE_CATALOG_VERSION = 11;

function scopeText(value) {
  return normalizedArticleValue(value).toLowerCase().replace(/[–—]/g, '-').replace(/\b(one|two|1|2)[ -]+step\b/g, (_, n) => `${({one:'1',two:'2'})[n] || n}-step`);
}

export function modelsMentioned(text, catalog = BASE_KNOWLEDGE_MODELS) {
  const haystack = scopeText(text);
  return catalog.filter((model) => ['current','previous'].includes(model.status) && model.aliases.some((alias) => {
    const phrase = scopeText(alias).trim();
    if (!phrase) return false;
    const escaped = phrase.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').replace(/\s+/g, '\\s+');
    // A one-word discovered label is too ambiguous on its own (for example,
    // “Daily” in “daily loss limit”). It counts only when the question/article
    // explicitly calls it an Account, model, or Challenge.
    const expression = !phrase.includes(' ')
      ? `\\b${escaped}\\s+(?:account|model|challenge)\\b`
      : `(?:^|[^a-z0-9])${escaped}(?=$|[^a-z0-9])`;
    return new RegExp(expression, 'i').test(haystack);
  }));
}

export async function getArticleScopeOverrides(sb = supabaseAdmin()) {
  try {
    const raw = await getRaw(sb, 'article_scope_overrides');
    const parsed = raw ? (typeof raw === 'string' ? JSON.parse(raw) : raw) : {};
    return parsed && typeof parsed === 'object' ? parsed : {};
  } catch { return {}; }
}

export async function saveArticleScopeOverride(articleId, changes, sb = supabaseAdmin()) {
  const id = String(articleId || '').trim();
  if (!id) throw new Error('Missing article identifier.');
  const product = ['cfd', 'futures'].includes(changes?.product) ? changes.product : null;
  const model = String(changes?.model || 'all').trim();
  if (!product) throw new Error('Choose CFD or Futures.');
  // The Admin selector supplies catalogue slugs. Validate the identifier
  // without rebuilding the full article catalogue; that rebuild was the
  // reason a single Save could remain pending until Vercel terminated it.
  if (model !== 'all' && !/^[a-z0-9][a-z0-9-]{0,159}$/i.test(model)) throw new Error('The selected Account model is invalid.');
  const overrides = await getArticleScopeOverrides(sb);
  overrides[id] = { product, model, updatedAt: new Date().toISOString() };
  const { data: savedRow, error } = await sb.from('settings').upsert({ key: 'article_scope_overrides', value: JSON.stringify(overrides) }, { onConflict: 'key' }).select('value').single();
  if (error) throw new Error('Could not save the article scope: ' + error.message);
  const savedOverrides = typeof savedRow?.value === 'string' ? JSON.parse(savedRow.value) : savedRow?.value;
  const savedScope = savedOverrides?.[id];
  if (!savedScope || savedScope.product !== product || savedScope.model !== model) throw new Error('The article scope could not be verified after saving. Nothing was reported as saved.');
  // Article scope corrections are consumed directly from
  // article_scope_overrides during retrieval. Rebuilding the complete
  // catalogue here made a one-row Admin save wait on thousands of articles
  // and could exceed the serverless request window.
  return { savedScope };
}

// Read beyond the database response cap. Advance by the actual returned count
// and require an empty page, so even a lower configured cap cannot truncate us.
async function readAllArticles(sb, columns, publishedOnly = false) {
  const rows = [];
  for (;;) {
    let query = sb.from('articles').select(columns).order('intercom_id', { ascending: true });
    if (publishedOnly) query = query.eq('state', 'published');
    const { data, error } = await query.range(rows.length, rows.length + 499);
    if (error) throw new Error('Reading complete article inventory failed: ' + error.message);
    if (!data?.length) return rows;
    rows.push(...data);
  }
}

export async function getScopeCatalog(sb = supabaseAdmin(), verifiedArticleOverrides = null) {
  const articles = await readAllArticles(sb, 'intercom_id,title,body,url', true);
  const articleOverrides = verifiedArticleOverrides || await getArticleScopeOverrides(sb);
  const raw = await getRaw(sb,'scope_model_overrides');
  const overrides = raw ? (typeof raw==='string'?JSON.parse(raw):raw) : {};
  let catalogue=BASE_KNOWLEDGE_MODELS.map(m=>({...m,articles:[],articleCount:0}));
  catalogue.push(...discoverModels(articles,catalogue));
  catalogue=applyModelOverrides(catalogue,overrides);
  // Count against the governed aliases, including merged names. A shared FAQ
  // can belong to several model evidence lists; it is never moved or deleted.
  for(const model of catalogue){
    if(!['current','previous'].includes(model.status))continue;
    const matched=articles.filter(article=>{
      const override=articleOverrides[String(article.intercom_id)];
      if(override)return override.product===model.product&&override.model===model.slug;
      return modelsMentioned(`${article.title||''}\n${article.body||''}`,[model]).length>0;
    });
    const evidence=[...new Map([...matched.map(a=>({id:String(a.intercom_id),title:a.title,url:a.url})),...(model.articles||[])].map(a=>[a.id,a])).values()];
    model.articleCount=evidence.length;model.articles=evidence.slice(0,80);
  }
  return {version:SCOPE_CATALOG_VERSION,generatedAt:new Date().toISOString(),articlesScanned:articles.length,
    products:[{slug:'cfd',name:'CFD'},{slug:'futures',name:'Futures'},{slug:'both',name:'Both'}],
    models:catalogue.sort((a,b)=>a.product.localeCompare(b.product)||a.name.localeCompare(b.name))};
}

export async function saveScopeModelOverride(slug, changes, sb = supabaseAdmin()) {
  const safeSlug = String(slug || '').trim();
  if (!safeSlug) throw new Error('Missing model identifier.');
  let overrides = {};
  try {
    const raw = await getRaw(sb, 'scope_model_overrides');
    overrides = raw ? (typeof raw === 'string' ? JSON.parse(raw) : raw) : {};
  } catch { overrides = {}; }
  overrides[safeSlug] = {
    ...(overrides[safeSlug] || {}),
    ...(['current', 'previous', 'review','rejected'].includes(changes?.status) ? { status: changes.status } : {}),
    ...(['cfd', 'futures'].includes(changes?.product) ? { product: changes.product } : {}),
    ...(String(changes?.name || '').trim() ? { name: String(changes.name).trim().slice(0, 120) } : {}),
    ...(Array.isArray(changes?.aliases)?{aliases:changes.aliases.map(String).filter(Boolean).slice(0,40)}:{}),
    ...(Array.isArray(changes?.articles)?{articles:changes.articles.slice(0,80)}:{})
  };
  const { data: savedRow, error } = await sb.from('settings').upsert({ key: 'scope_model_overrides', value: JSON.stringify(overrides) }, { onConflict: 'key' }).select('value').single();
  if (error) throw new Error('Could not save the catalogue correction: ' + error.message);
  const savedOverrides = typeof savedRow?.value === 'string' ? JSON.parse(savedRow.value) : savedRow?.value;
  const savedStatus = savedOverrides?.[safeSlug]?.status;
  if (changes?.status && savedStatus !== changes.status) throw new Error('The catalogue status could not be verified after saving.');
  const catalog = await publishScopeCatalog(sb);
  return { catalog, savedStatus };
}

export async function getPublishedScopeCatalog(sb = supabaseAdmin()) {
  const saved = await getRaw(sb, 'published_scope_catalog');
  if (saved) {
    try {
      const parsed = typeof saved === 'string' ? JSON.parse(saved) : saved;
      if (parsed?.version === SCOPE_CATALOG_VERSION) return parsed;
    } catch { /* rebuild below */ }
  }
  return publishScopeCatalog(sb);
}

export async function publishScopeCatalog(sb = supabaseAdmin(), verifiedArticleOverrides = null) {
  try {
  const catalog = await getScopeCatalog(sb, verifiedArticleOverrides);
  const { error } = await sb.from('settings').upsert({ key: 'published_scope_catalog', value: JSON.stringify(catalog) });
  if (error) throw new Error('Could not publish the verified Account catalogue: ' + error.message);
  const result=await sb.from('settings').upsert({key:'model_discovery_last_scan',value:JSON.stringify({at:catalog.generatedAt,status:'complete',articlesScanned:catalog.articlesScanned,candidates:catalog.models.filter(m=>m.status==='review').length})});
  if(result.error) throw result.error;
  await logActivity({actorRole:'system',eventType:'model_discovery',success:true,metadata:{articlesScanned:catalog.articlesScanned,candidates:catalog.models.filter(m=>m.status==='review').length}});
  return catalog;
  } catch(error) {
    await sb.from('settings').upsert({key:'model_discovery_last_scan',value:JSON.stringify({at:new Date().toISOString(),status:'failed',error:error.message})});
    await logActivity({actorRole:'system',eventType:'model_discovery',success:false,metadata:{error:error.message}});
    throw error;
  }
}
...
const ARTICLE_REGION_SCOPES_KEY = 'article_region_scopes_v1';
// Verified from the live USA Relaunch collection on 2026-09-23. This protects
// the first request after deployment; the next knowledge sync replaces it with
// hierarchy-derived data for every regional Help Center collection.
const BOOTSTRAP_ARTICLE_REGION_SCOPES = Object.fromEntries([
  '12673271', '12673301', '12673362', '12673505'
].map((id) => [id, { regions: ['us'], labels: ['United States'], collectionPath: ['USA Relaunch'], source: 'verified_bootstrap_until_first_sync' }]));
const REGION_DEFINITIONS = [
  { code: 'us', label: 'United States', patterns: [/\busa\b/i, /\bunited states\b/i, /\b(?:us|u\.s\.?)\s+(?:clients?|traders?|customers?|residents?|accounts?|market|relaunch)\b/i, /\b(?:in|from|for|within)\s+(?:the\s+)?(?:us|u\.s\.?|usa|united states)\b/i], exclusions: [/\bnon[- ]?(?:us|u\.s\.?|usa)\b/i, /\bnot\s+(?:a\s+)?(?:us|u\.s\.?|usa|united states)\s+(?:client|trader|customer|resident)\b/i, /\boutside\s+(?:the\s+)?(?:us|u\.s\.?|usa|united states)\b/i] },
  { code: 'uk', label: 'United Kingdom', patterns: [/\bunited kingdom\b/i, /\b(?:uk|u\.k\.?)\s+(?:clients?|traders?|customers?|residents?|accounts?|market)\b/i] },
  { code: 'eu', label: 'European Union', patterns: [/\beuropean union\b/i, /\beu\s+(?:clients?|traders?|customers?|residents?|accounts?|market)\b/i] },
  { code: 'ca', label: 'Canada', patterns: [/\bcanada\b/i, /\bcanadian\s+(?:clients?|traders?|customers?|residents?|accounts?|market)\b/i] },
  { code: 'au', label: 'Australia', patterns: [/\baustralia\b/i, /\baustralian\s+(?:clients?|traders?|customers?|residents?|accounts?|market)\b/i] },
  { code: 'ae', label: 'United Arab Emirates', patterns: [/\bunited arab emirates\b/i, /\buae\b/i] },
  { code: 'in', label: 'India', patterns: [/\bindia\b/i, /\bindian\s+(?:clients?|traders?|customers?|residents?|accounts?|market)\b/i] },
  { code: 'pk', label: 'Pakistan', patterns: [/\bpakistan\b/i, /\bpakistani\s+(?:clients?|traders?|customers?|residents?|accounts?|market)\b/i] },
  { code: 'bd', label: 'Bangladesh', patterns: [/\bbangladesh\b/i, /\bbangladeshi\s+(?:clients?|traders?|customers?|residents?|accounts?|market)\b/i] },
  { code: 'latam', label: 'Latin America', patterns: [/\blatin america\b/i, /\blatam\b/i] }
];

export function regionScopesInText(value) {
  const text = String(value || '');
  return REGION_DEFINITIONS.filter((region) => region.patterns.some((pattern) => pattern.test(text)))
    .map(({ code, label }) => ({ code, label }));
}

export function questionRegionIntent(value) {
  const text = String(value || '');
  const included = [], excluded = [];
  for (const region of REGION_DEFINITIONS) {
    if ((region.exclusions || []).some((pattern) => pattern.test(text))) excluded.push(region.code);
    else if (region.patterns.some((pattern) => pattern.test(text))) included.push(region.code);
  }
  return { included, excluded, explicit: included.length > 0 || excluded.length > 0 };
}

export async function getArticleRegionScopes(sb = supabaseAdmin()) {
  try {
    const raw = await getRaw(sb, ARTICLE_REGION_SCOPES_KEY);
    if (raw == null) return BOOTSTRAP_ARTICLE_REGION_SCOPES;
    const parsed = raw ? (typeof raw === 'string' ? JSON.parse(raw) : raw) : {};
    return parsed && typeof parsed === 'object' ? parsed : {};
  } catch { return BOOTSTRAP_ARTICLE_REGION_SCOPES; }
}

async function fetchAllHelpCenterGroups(token, kind) {
  // Intercom's Help Center hierarchy endpoints are not exposed under every
  // API version even when the Articles endpoint is. Negotiate only for these
  // endpoints; article syncing continues to use the current version.
  const versions = kind === 'sections' ? ['2.9', '2.8', '2.7'] : ['2.11', '2.9', '2.8', '2.7'];
  const byId = new Map();
  let page = 1, totalPages = 1, selectedVersion = null;
  do {
    const url = new URL(`https://api.intercom.io/help_center/${kind}`);
    url.searchParams.set('per_page', '150');
    url.searchParams.set('page', String(page));
    let response = null, lastFailure = '';
    const candidates = selectedVersion ? [selectedVersion] : versions;
    for (const version of candidates) {
      response = await fetchWithTimeout(url.toString(), { headers: {
        Authorization: `Bearer ${token}`, Accept: 'application/json', 'Intercom-Version': version
      } }, 15000, `Intercom ${kind} took too long to respond`);
      if (response.ok) { selectedVersion = version; break; }
      lastFailure = `${response.status}. ${(await response.text()).slice(0, 200)}`;
      // Authentication, permission, and rate-limit failures are not version
      // compatibility failures and must not be multiplied across versions.
      if (![400, 404].includes(response.status)) break;
    }
    if (!response?.ok) throw new Error(`Intercom ${kind} returned ${lastFailure || 'an unknown error'}`);
    const json = await response.json();
    for (const item of (Array.isArray(json?.data) ? json.data : [])) if (item?.id != null) byId.set(String(item.id), item);
    totalPages = Math.max(1, Number(json?.pages?.total_pages || 1));
    page++;
  } while (page <= totalPages);
  return [...byId.values()];
}

export function buildArticleRegionScopes(articles, collections, sections) {
  const groups = new Map();
  for (const group of [...(collections || []), ...(sections || [])]) {
    if (group?.id != null) groups.set(String(group.id), group);
  }
  const pathFor = (article) => {
    const path = [], visited = new Set();
    const seeds = [...(Array.isArray(article?.parent_ids) ? article.parent_ids : []), article?.parent_id].filter((id) => id != null).map(String);
    const visit = (id) => {
      if (!id || visited.has(id)) return;
      visited.add(id);
      const group = groups.get(id);
      if (!group) return;
      visit(group.parent_id == null ? '' : String(group.parent_id));
      if (group.name) path.push(String(group.name));
    };
  ...
  for (const article of (articles || [])) {
    ...
    const collectionPath = pathFor(article);
    const regions = regionScopesInText(collectionPath.join(' > '));
    if (regions.length) scoped[String(article.id)] = {
      regions: regions.map((region) => region.code),
      labels: regions.map((region) => region.label),
      collectionPath,
      source: 'intercom_help_center_hierarchy'
    };
  }
  return scoped;
}
...
export async function fetchAllPublishedArticles(token) {
  ...
  const fetchPage = async (page) => {
    ...
    }, 15000, 'Intercom took too long to respond');
    if (!res.ok) {
  ...
  for (const article of articlesFrom(first)) if (article?.id != null) byId.set(String(article.id), article);
  const totalPages = Math.max(1, Number(first?.pages?.total_pages || 1));
  const expectedTotal = Number(first?.total_count || 0);

  // Page-number pagination is supported by this Intercom endpoint. Fetch a few
  // pages concurrently so a 900+ article comparison stays within Vercel's
  // request window instead of waiting on every network round-trip in series.
  let nextPage = 2;
  const worker = async () => {
    while (nextPage <= totalPages) {
      const page = nextPage++;
      const json = await fetchPage(page);
      for (const article of articlesFrom(json)) if (article?.id != null) byId.set(String(article.id), article);
    }
  };
  await Promise.all(Array.from({ length: Math.min(4, Math.max(0, totalPages - 1)) }, worker));

  // Never compare or delete against a partial Intercom scan.
  if (expectedTotal > 0 && byId.size < expectedTotal) {
    throw new Error(`Intercom scan was incomplete (${byId.size} of ${expectedTotal} articles returned). No knowledge-base changes were applied.`);
  }
...
function normalizedArticleValue(value) {
  return htmlToText(value || '').replace(/\s+/g, ' ').trim();
}

// Fingerprint only searchable FAQ content. Source URL formatting can vary
// without the article text changing and must not regenerate embeddings.
function articleContentHash(article) {
  return sha256(JSON.stringify({
    title: normalizedArticleValue(article?.title),
    body: normalizedArticleValue(article?.body)
  }));
}
...
export async function openaiEmbed(openaiKey, inputs) {
  ...
  }, 30000, 'OpenAI embeddings took too long to respond');
  if (!res.ok) {
    const t = await res.text();
...
export async function openaiChatDetailed(apiKey, model, messages, baseUrl = 'https://api.openai.com/v1') {
  ...
  // Support answers and helper decisions should be repeatable. OpenAI's o-series
  // models do not accept this option, so leave it out only for those models.
  if (!/^o\d/i.test(String(model || ''))) body.temperature = 0;
  ...
  }, 45000, `${baseUrl.includes('groq.com') ? 'Groq' : 'OpenAI'} took too long to respond`);
  if (!res.ok) {
    const t = await res.text();
...
async function fetchWithTimeout(url, options = {}, timeoutMs = 30000, timeoutMessage = 'The service took too long to respond') {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), timeoutMs);
  try {
    return await fetch(url, { ...options, signal: controller.signal });
  } catch (error) {
    if (error?.name === 'AbortError') throw new Error(`${timeoutMessage}. Please retry; completed sync progress is safe.`);
    throw error;
...
export const CONCEPT_GROUPS = [
  ...
  {
    id: 'withdraw_method',
    match: ['withdrawal method', 'withdrawal methods', 'payout method', 'payout methods', 'minimum withdrawal', 'maximum withdrawal', 'withdrawal fee', 'payout fee', 'usdt', 'usdc', 'crypto withdrawal', 'bank transfer', 'riseworks', 'erc20', 'trc20'],
    expand: ['withdrawal methods crypto USDT USDC RiseWorks minimum maximum', 'payout method processing times and limits']
  },
  {
    id: 'targets',
    match: ['profit target', 'target profit', 'pass the challenge', 'passing the challenge', 'challenge phase', 'evaluation phase', 'minimum trading days', 'consistency rule'],
    expand: ['profit target and minimum trading days to pass', 'evaluation phase requirements consistency rule']
  },
  {
    id: 'drawdown',
    match: ['drawdown', 'mll', 'maximum loss limit', 'loss limit', 'daily loss', 'max loss', 'breach', 'violation', 'rule break', 'lost account', 'failed'],
    expand: ['daily loss limit and maximum loss limit rules', 'what breaches an account drawdown rules']
  },
  {
    id: 'account_reset',
    match: ['reset', 'restart account', 'start over', 'breached account', 'breach my account', 'reset fee', 'reset price', 'without payment', 'without paying'],
    expand: ['challenge account reset eligibility and reset fee by account model and size', 'Stellar Lite Challenge Phase 1 reset price after breach', 'what happens after daily loss breach and how to reset the Challenge Account']
  },
  {
    id: 'scaling',
  ...
  {
    id: 'kyc_country',
    match: ['kyc country', 'kyc compliance', 'citizenship', 'nationality', 'passport accepted', 'residence permit', 'restricted country', 'restricted countries'],
    expand: ['restricted countries citizenship nationality KYC eligibility', 'accepted KYC identity documents by country', 'country restriction passport residence permit compliance']
  }
];
...
export async function clarifyQuery({ question, provider, model, openaiKey, groqKey }) {
  ...
  const sys =
    ...
    '"emotion" (exactly one of "neutral", "confused", "concerned", "frustrated", "urgent", "eager", or "disappointed"; detect the CLIENT tone inside pasted messages, not an Agent instruction wrapped around it), ' +
    '"needs_clarification" (true only when information the USER must supply is missing and different values require different FAQ rules), ' +
    '"clarifying_question" (one short question naming the exact missing detail), ' +
    '"clarification_reason" (one sentence explaining why that missing detail changes the rule), ' +
    '"choices" (2-5 objects with "value", "label", and "description"). ' +
    'A choice is a user attribute or intent, NEVER a possible policy answer. Never put facts, limits, percentages, money amounts, permissions, predictions, or conclusions in choices. ' +
    'Do not ask for clarification when the user supplied the relevant detail, when all interpretations can be answered safely, or when the message contains multiple explicit questions: preserve and answer every part. ' +
    'Ask whenever a missing Account model, platform, phase, timeframe, transaction type, calculation value, or intended meaning would change the factual answer. ' +
    'Choice labels must be short, self-explanatory, mutually exclusive user selections; descriptions explain when the user should choose them without revealing an answer. ' +
    'Never depend on exact wording; semantically equivalent questions must produce the same clarification decision. ' +
    ...
    'The words payment or payments do NOT mean payout when the customer is asking about a purchase, reset, fee, charge, or account breach. ' +
    'A question that already names an Account model, phase, event, and requested outcome is self-contained and must be answered directly, not clarified.';
  try {
    ...
    return {
      ...
      emotion: ['neutral', 'confused', 'concerned', 'frustrated', 'urgent', 'eager', 'disappointed'].includes(parsed.emotion) ? parsed.emotion : 'neutral',
      ...
      clarificationReason: typeof parsed.clarification_reason === 'string' ? parsed.clarification_reason.slice(0, 300) : '',
      choices: Array.isArray(parsed.choices) ? parsed.choices.map((choice) => ({
        value: typeof choice?.value === 'string' ? choice.value.trim().slice(0, 180) : '',
        label: typeof choice?.label === 'string' ? choice.label.trim().slice(0, 120) : '',
        description: typeof choice?.description === 'string' ? choice.description.trim().slice(0, 220) : ''
      })).filter((choice) => choice.value && choice.label).slice(0, 5) : []
    };
  } catch { return null; }
}
...
export async function syncStep(sb, { intercomToken, openaiKey }) {
  ...
  // Keep one indexing response well inside the serverless request limit even
  // when an article is large or the embeddings service is busy.
  const PROCESS_BATCH = 3, EMBED_BATCH = 48, INSERT_BATCH = 50;
  ...
  if (todo && todo.length) {
    ...
    return {
      ...
      // Reaching zero queued rows is not completion. The next step must perform
      // a fresh Intercom comparison and explicitly confirm zero differences.
      done: false, verificationRequired: (count || 0) === 0,
      sampleTitles: todo.slice(0, 5).map((a) => a.title || '(untitled)'), chunkCount: rows.length,
      savedRows: rows.length, embeddingBatches: Math.ceil(flat.length / EMBED_BATCH), durationMs: Date.now() - started
    };
  }

  // Article text alone does not reveal when a policy belongs to a regional
  // Help Center collection (for example USA Relaunch). Preserve that hierarchy
  // separately so retrieval cannot generalize a regional exception worldwide.
  const articles = await fetchAllPublishedArticles(intercomToken);
  let articleRegionScopes = await getArticleRegionScopes(sb);
  let regionScopeStatus = 'preserved';
  let regionScopeWarning = null;
  try {
    const [collections, sections] = await Promise.all([
      fetchAllHelpCenterGroups(intercomToken, 'collections'),
      fetchAllHelpCenterGroups(intercomToken, 'sections')
    ]);
    articleRegionScopes = buildArticleRegionScopes(articles, collections, sections);
    const { error: regionScopeError } = await sb.from('settings').upsert({
      key: ARTICLE_REGION_SCOPES_KEY,
      value: JSON.stringify(articleRegionScopes)
    }, { onConflict: 'key' });
    if (regionScopeError) throw new Error('Saving Help Center regional scope failed: ' + regionScopeError.message);
    regionScopeStatus = 'refreshed';
  } catch (error) {
    // Geographic metadata is a safety enhancement, not a prerequisite for
    // keeping the verified article corpus current. Retain the last known-good
    // map (or the built-in USA bootstrap) and continue the article sync.
    regionScopeWarning = String(error?.message || error);
    console.warn('Help Center regional hierarchy refresh skipped:', regionScopeWarning);
  }
  const liveIds = new Set(articles.map((a) => String(a.id)));
  const stored = await readAllArticles(sb, 'intercom_id,title,body,url,content_hash');
  // Supabase may return a numeric database ID while Intercom returns the same
  // ID as a string. Normalise both sides or every run can mistake existing
  // articles for new ones.
  const storedMap = new Map((stored || []).map((r) => [String(r.intercom_id), r]));
  ...
  const flagged = [];
  let newFound = 0, updatedFound = 0;
  for (const a of articles) {
    const id = String(a.id);
    const storedArticle = storedMap.get(id);
    const hash = articleContentHash(a);
    const searchableContentMatches = storedArticle &&
      normalizedArticleValue(storedArticle.title) === normalizedArticleValue(a.title) &&
      normalizedArticleValue(storedArticle.body) === normalizedArticleValue(a.body);
    // Compare the stored and current text directly. This remains correct even
    // when the fingerprint algorithm changes, and ignores URL-only variation.
    if (!searchableContentMatches) {
      if (storedArticle) updatedFound++; else newFound++;
      flagged.push({
  ...
  // Publish selector changes only after a complete comparison finds no pending
  // article work. A timeout, cancellation, or partially indexed queue therefore
  // cannot alter the model list seen by Agents.
  if (flagged.length === 0) await publishScopeCatalog(sb);
  return {
    ...
    changedFound: flagged.length, newFound, updatedFound, storedBefore: storedCount,
    comparisonConfirmed: flagged.length === 0, deletionGuard: safeToDelete ? 'passed' : 'activated',
    regionScopedArticles: Object.keys(articleRegionScopes).length,
    regionScopeStatus, regionScopeWarning,
    durationMs: Date.now() - started
  };
}

// The browser's manual checker and Vercel Cron can arrive together. A database
// lease, rather than an in-memory flag, survives separate serverless instances
// and automatically expires if an invocation is killed mid-run.
export async function withSyncLease(sb, trigger, work) {
  const token = crypto.randomUUID();
  const { data: acquired, error } = await sb.rpc('claim_sync_lease', {
    p_token: token, p_trigger: trigger, p_lease_seconds: 120
  });
  if (error) throw new Error(`Sync reliability setup is required: ${error.message}`);
  if (!acquired) return { locked: true, reason: 'Another sync is already processing the same knowledge queue.' };
  try {
    return await work();
  } finally {
    // Release errors are safe: the 120-second expiry still prevents a permanent
    // lock if the final request is interrupted.
    try { await sb.rpc('release_sync_lease', { p_token: token }); } catch { /* lease expiry is the fallback */ }
  }
}
  ...
  if (!error && done) {
    try {
      await sb.from('settings').upsert([
        { key: trigger === 'auto' ? 'last_auto_sync_at' : 'last_manual_sync_at', value: finishedAt.toISOString() },
        { key: `${trigger === 'auto' ? 'last_auto' : 'last_manual'}_sync_summary`, value: JSON.stringify({ at: finishedAt.toISOString(), trigger, status, changed: agg.articlesChanged, indexed: agg.articlesIndexed, deleted: agg.articlesDeleted }) }
      ]);
    } catch { /* ignore */ }
  }
...
export async function getAutoSyncConfig(sb = supabaseAdmin()) {
  ...
  const lastManualSyncAt = await getRaw(sb, 'last_manual_sync_at');
  let lastSummary = null;
  let lastManualSummary = null;
  try { const s = await getRaw(sb, 'last_auto_sync_summary') || await getRaw(sb, 'last_sync_summary'); lastSummary = s ? JSON.parse(s) : null; } catch { lastSummary = null; }
  try { const s = await getRaw(sb, 'last_manual_sync_summary'); lastManualSummary = s ? JSON.parse(s) : null; } catch { lastManualSummary = null; }
  return { enabled, intervalHours, intervalOptions: AUTO_SYNC_INTERVALS, lastAutoSyncAt, lastManualSyncAt, lastSummary, lastManualSummary };
}
...
export async function getLastSyncMarkers(sb = supabaseAdmin()) {
  ...
  let lastSummary = null;
  try { const s = await getRaw(sb, 'last_auto_sync_summary') || await getRaw(sb, 'last_sync_summary'); lastSummary = s ? JSON.parse(s) : null; } catch { lastSummary = null; }
  return { lastAutoSyncAt, lastManualSyncAt, lastSummary };
}
...
export async function knowledgeStatus(sb = supabaseAdmin()) {
  const [docs, articleOverrides, articleRows] = await Promise.all([
    listKnowledge(sb), getArticleScopeOverrides(sb),
    readAllArticles(sb, 'intercom_id,title,url,body', true)
  ]);
  // Admin must see current article titles and assignments, never a cached
  // catalogue snapshot left behind by an earlier deployment.
  const scopeCatalog = await getScopeCatalog(sb, articleOverrides);
  const articleScopes = articleRows.sort((a, b) => String(a.title || '').localeCompare(String(b.title || ''))).map((article) => {
    const override = articleOverrides[String(article.intercom_id)];
    const detected = modelsMentioned(`${article.title || ''}\n${article.body || ''}`, scopeCatalog.models);
    const url = String(article.url || '');
    const product = override?.product || (/helpfutures\.fundednext\.com/i.test(url) || detected.some((item) => item.product === 'futures') ? 'futures' : 'cfd');
    const sameFamily = detected.filter((item) => item.product === product);
    return { id: String(article.intercom_id), title: article.title || 'Untitled FAQ', url, product, model: override?.model || (sameFamily.length === 1 ? sameFamily[0].slug : 'all'), overridden: !!override };
  });
  let indexedChunks = 0;
  try {
  ...
  return { docs, indexedChunks, indexedAt, scopeCatalog, articleScopes };
}
