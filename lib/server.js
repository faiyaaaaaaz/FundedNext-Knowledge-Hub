...
// Fingerprint the exact information that is searchable or shown as source
// metadata. HTML formatting and whitespace-only differences do not cause a
// needless re-index, while real title, body, or URL changes do.
function articleContentHash(article) {
  return sha256(JSON.stringify({
    ...
    url: normalizedArticleUrl(article?.url)
  }));
}
...
export async function syncStep(sb, { intercomToken, openaiKey }) {
  ...
  const { data: stored, error: e1 } = await sb.from('articles').select('intercom_id,title,url,content_hash');
  ...
  for (const a of articles) {
    ...
    // Accept the previous raw-HTML fingerprint when it still matches. This
    // prevents this upgrade itself from queueing every existing article once.
    const legacyHash = sha256((a.title || '') + '\n' + (a.body || ''));
    const hashMatches = storedArticle && (storedArticle.content_hash === hash || storedArticle.content_hash === legacyHash);
    const metadataMatches = storedArticle &&
      normalizedArticleValue(storedArticle.title) === normalizedArticleValue(a.title) &&
      normalizedArticleUrl(storedArticle.url) === normalizedArticleUrl(a.url);
