...
function _computeOne(item) {
  ...
  const ref = `${inst.symbol} values from the calculator: pip value $${inst.pv} per lot, pip size ${inst.p}, contract size ${inst.c}${inst.cf !== 1 ? `, conversion factor ${inst.cf}` : ''}.`;
  let leverage = null;
  if (item.type === 'margin' || item.type === 'maxlot') {
    const lv = calcResolveLeverage({ manualLeverage: item.manualLeverage, accountModel: item.accountModel, marketType: inst.mt });
...
`To size a stop for ${inst.symbol} on a $${_fMoney(item.balance)} account with a ${_fNum(item.slPips, 2)}-pip stop, here is the method so you can do it yourself:
Step 1 — Risk in USD = Balance × (Risk% ÷ 100). Choose a Risk% that respects your account's own max-risk rule.
Step 2 — Lot Size = Risk in USD ÷ (Pip Value × Stop-Loss pips) = Risk ÷ ($${inst.pv} × ${_fNum(item.slPips, 0)}).
Worked example at 1% risk: Risk = $${_fMoney(oneP)}, Lot = $${_fMoney(oneP)} ÷ ($${inst.pv} × ${_fNum(item.slPips, 0)}) = ${_fNum(oneLot, 2)} lots. (${ref})
Tell me the exact Risk% you want and I'll give the precise lot size and stop-loss price.` };
    }
...
export async function runCalculators({ question, provider, model, openaiKey, groqKey }) {
  ...
  const results = ex.calculations.map(_computeOne).filter(Boolean);
