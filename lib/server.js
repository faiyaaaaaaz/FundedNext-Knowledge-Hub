...
function signSession({ role, version, gv, sessionId, name, email, authProvider, userId }) {
  const payload = b64url(JSON.stringify({
    ...
    // Require a fresh Google-backed app session five days after sign-in.
...
export async function authenticateRequest(req) {
  ...
  if (token) {
    try {
      ...
      // Password and other legacy sessions are invalid immediately. Re-check
      // both domain and current Admin allowlist on every request so removing an
      // email from Vercel removes its elevated access without a code change.
