...
export async function saveArticleScopeOverride(articleId, changes, sb = supabaseAdmin()) {
  ...
  const validationCatalog = await getScopeCatalog(sb);
  if (model !== 'all' && !validationCatalog.models.some((item) => item.slug === model && item.product === product)) {
    throw new Error('The selected Account model does not belong to this product.');
  }
