...
export async function saveArticleScopeOverride(articleId, changes, sb = supabaseAdmin()) {
  ...
  if (model !== 'all' && !catalog.models.some((item) => item.slug === model && item.product === product)) {
  ...
  const { error } = await sb.from('settings').upsert({ key: 'article_scope_overrides', value: JSON.stringify(overrides) });
...
export async function saveScopeModelOverride(slug, changes, sb = supabaseAdmin()) {
  ...
  const { error } = await sb.from('settings').upsert({ key: 'scope_model_overrides', value: JSON.stringify(overrides) });
