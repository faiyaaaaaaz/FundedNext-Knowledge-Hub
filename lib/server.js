...
export async function getScopeCatalog(sb = supabaseAdmin()) {
  ...
  const articles = data || [];
  const articleOverrides = await getArticleScopeOverrides(sb);
...
export async function publishScopeCatalog(sb = supabaseAdmin()) {
  const catalog = await getScopeCatalog(sb);
