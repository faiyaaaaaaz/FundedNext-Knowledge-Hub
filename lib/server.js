...
export const BASE_KNOWLEDGE_MODELS = [
  ...
  { slug: 'rapid-challenge', product: 'cfd', name: 'Rapid Challenge', status: 'current', aliases: ['rapid challenge', 'rapid account'] },
  { slug: 'futures-challenge', product: 'futures', name: 'Futures Challenge', status: 'current', aliases: ['futures challenge'] },
  { slug: 'futures-fundednext', product: 'futures', name: 'Futures FundedNext Account', status: 'current', aliases: ['futures fundednext account', 'fundednext futures account', 'funded futures account'] }
];
...
export async function getScopeCatalog(sb = supabaseAdmin()) {
  const { data, error } = await sb.from('articles').select('intercom_id,title,body').eq('state', 'published').limit(5000);
  if (error) throw new Error('Could not build the Account catalogue: ' + error.message);
  const articles = data || [];
  const catalogue = BASE_KNOWLEDGE_MODELS.map((model) => ({ ...model, articleCount: 0, examples: [] }));
  for (const article of articles) {
    const text = `${article.title || ''}\n${article.body || ''}`;
    for (const model of modelsMentioned(text, catalogue)) {
      model.articleCount++;
      if (model.examples.length < 3 && article.title && !model.examples.includes(article.title)) model.examples.push(article.title);
    }
  }
  ...
  for (const article of articles) {
    ...
    const product = /\bfutures?\b/i.test(title) ? 'futures' : 'cfd';
    const patterns = [
    ...
    for (const pattern of patterns) {
      for (const match of title.matchAll(pattern)) {
        ...
        if (ignored.test(name) || modelsMentioned(name, catalogue).length) continue;
        const key = `${product}:${name.toLowerCase()}`;
        const candidate = candidates.get(key) || { name, product, titles: new Set(), articleIds: new Set() };
        candidate.titles.add(title); candidate.articleIds.add(String(article.intercom_id)); candidates.set(key, candidate);
      }
    }
  }
  for (const candidate of candidates.values()) {
    const articleCount = candidate.articleIds.size;
    catalogue.push({
      ...
      examples: [...candidate.titles].slice(0, 3), autoDetected: true
    });
  }
  let overrides = {};
  try {
...
export async function getPublishedScopeCatalog(sb = supabaseAdmin()) {
  ...
  if (saved) {
    try { return typeof saved === 'string' ? JSON.parse(saved) : saved; } catch { /* rebuild below */ }
  }
