...
function calcFindInstrument(name) {
  if (!name) return null;
  const key = String(name).toUpperCase().replace(/[^A-Z0-9]/g, '');
  const resolved = _CALC_ALIASES[key] || key;
  if (CALC_INSTRUMENTS[resolved]) return { symbol: resolved, ...CALC_INSTRUMENTS[resolved] };
  return null;
}
...
function calcResolveLeverage({ manualLeverage, accountModel, marketType }) {
  if (manualLeverage && Number(manualLeverage) > 0) return { leverage: Number(manualLeverage) };
  if (marketType === 'Crypto') return { leverage: 1 };
  const step = _calcStep(accountModel);
  if (!step) return { unknown: true };
  const byMarket = CALC_LEVERAGE[step] && CALC_LEVERAGE[step][marketType];
...
async function calcExtract({ question, provider, model, openaiKey, groqKey }) {
  ...
  if (!key) return null;
  const sys =
    'Extract trade-calculator parameters from the question. Return STRICT JSON only. Keys: ' +
    'type (one of "margin","maxlot","pnl","pips","risk_money","risk_percent","none"), instrument, accountModel, ' +
    'manualLeverage, price, lot, balance, position ("buy"/"sell"), entry, close, risk, reward, riskPct, rewardPct, slPips, action ("add"/"remove"), amount. ' +
    'Numbers must be plain numbers or null; strings or null. ' +
    'Use type "none" if the question is about rules/policy/eligibility rather than a numeric calculation the calculator performs. ' +
    ...
    'pips needs open+action+amount; risk_money needs entry+lot+risk+reward+position; risk_percent needs balance+entry+riskPct+slPips+position. ' +
    'instrument should be the trading symbol (e.g. EURUSD, XAUUSD). accountModel is the raw account text (e.g. "Stellar 1-step challenge").';
  try {
    const { content } = await openaiChatDetailed(key, model, [
      { role: 'system', content: sys },
...
// Main entry. Returns { handled:true, answer, calcType, sourceTitle, confidence, segments } or null.
export async function tryCalculator({ question, provider, model, openaiKey, groqKey }) {
  if (!_CALC_HINT.test(question || '')) return null;
  const ex = await calcExtract({ question, provider, model, openaiKey, groqKey });
  if (!ex || !ex.type || ex.type === 'none' || !CALC_ENGINES[ex.type]) return null;

  const inst = calcFindInstrument(ex.instrument);
  const title = CALC_TITLES[ex.type];
  const wrap = (answer, confidence) => ({
    handled: true, calcType: ex.type, sourceTitle: title, confidence,
    answer, segments: answer.split(/\n{2,}/).map((t) => t.trim()).filter(Boolean).map((t) => ({ text: t, refs: [1] }))
  });

  if (!inst) {
    return wrap(`To work out this ${ex.type.replace('_', ' ')} with the trade calculator I need a supported instrument symbol (for example EURUSD or XAUUSD). Which instrument is this for?`, 60);
  }

  let leverage = null;
  if (ex.type === 'margin' || ex.type === 'maxlot') {
    const lv = calcResolveLeverage({ manualLeverage: ex.manualLeverage, accountModel: ex.accountModel, marketType: inst.mt });
    if (lv.leverage != null) leverage = lv.leverage;
    else if (lv.ambiguous) {
      return wrap(`For ${inst.symbol} (${inst.mt}) the leverage depends on whether it's the Challenge or the Funded phase of that account, and they differ. Which one is it, or what manual leverage should I use?`, 60);
    }
  }

  const result = CALC_ENGINES[ex.type](ex, inst, leverage);
  if (!result.ok) {
    return wrap(`To calculate the ${ex.type.replace('_', ' ')} for ${inst.symbol} I still need ${result.missing.join(', ')}. Once you share that I can give the exact figure.`, 62);
  }
  return wrap(result.macro, 96);
}
