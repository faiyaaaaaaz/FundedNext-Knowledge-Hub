...
export async function saveArticleScopeOverride(articleId, changes, sb = supabaseAdmin()) {
  ...
  const catalog = await publishScopeCatalog(sb, savedOverrides);
  return { catalog, savedScope };
}
...
export async function knowledgeStatus(sb = supabaseAdmin()) {
  const [docs, scopeCatalog, articleOverrides, articleRows] = await Promise.all([
    listKnowledge(sb), getPublishedScopeCatalog(sb), getArticleScopeOverrides(sb),
