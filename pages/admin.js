...
function DataLoader({ title, detail }) {
  return <div className="page-loading-overlay" role="status" aria-live="polite"><div className="real-loader"><span className="loader-mark" aria-hidden="true" /><div><b>{title}</b><small>{detail}</small></div></div></div>;
}
...
export default function Admin() {
  ...
  const [snippetUsageDownloading, setSnippetUsageDownloading] = useState(false);
  const [diagnosticsDownloading, setDiagnosticsDownloading] = useState(false);
  ...
  async function loadSnippetUsage(offset = 0, append = false) {
    try {
      const response = await fetch(`/api/snippet-logs?offset=${offset}&limit=30`, { headers: headers() });
      if (handleAuthLoss(response)) return;
      const data = await response.json(); if (!response.ok) throw new Error(data.error);
      setSnippetUsage((current) => append ? { ...data, events: [...(current?.events || []), ...(data.events || [])] } : data);
    } catch (e) { setError(e.message); }
  }

  async function downloadGzip(path, filename, setter) {
    setter(true); clearMessages();
    try {
      const response = await fetch(path, { headers: headers() });
      if (handleAuthLoss(response)) return;
      if (!response.ok) { let data = {}; try { data = await response.json(); } catch {} throw new Error(data.error || `Download failed (HTTP ${response.status}).`); }
      const blob = await response.blob(); const url = URL.createObjectURL(blob);
      const link = document.createElement('a'); link.href = url; link.download = filename;
      document.body.appendChild(link); link.click(); link.remove(); setTimeout(() => URL.revokeObjectURL(url), 1000);
      setNotice('Diagnostic download is ready.');
    } catch (e) { setError(e.message); } finally { setter(false); }
  }
  ...
  const navigation = [
    ...
    ['disputes', '⚑', 'Disputes'], ['snippets', '⌘', 'Snippets'], ['snippetlogs', '↳', 'Snippet usage logs'], ['knowledge', '▤', 'Knowledge'], ['brain', '◉', 'FAQ explorer'], ['noticebrain', '◌', 'Notices explorer'], ['querylogs', '◧', 'Query & answer logs'], ['reports', '⇩', 'Downloads & diagnostics'], ['calcdata', '∑', 'Calculator data'], ['activity', '◫', 'Activity logs'],
  ...
  return (
    <main className="admin-shell workspace-admin-v2">
      ...
      <section className="admin-main">
        ...
        {tab === 'snippetlogs' && <div className="settings-stack snippet-usage-page">{snippetUsage && <><div className="activity-kpis">{[['Snippet uses', snippetUsage.summary.uses], ['Corrective snippets used', snippetUsage.summary.snippets], ['Queries affected', snippetUsage.summary.queriesAffected], ['Team members', snippetUsage.summary.users]].map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div><section className="settings-card"><div className="settings-head"><div><h2>Usage by corrective snippet</h2><p>Each trace records the exact question, scope, correction version, match score, and query log ID. Snippet injection is observable; model influence is deliberately not claimed.</p></div><div className="row"><button className="btn btn-secondary" onClick={() => loadSnippetUsage()}>Refresh</button><button className="btn btn-primary compact-download" disabled={snippetUsageDownloading} onClick={() => downloadGzip('/api/snippet-logs?download=1', 'fundednext-snippet-usage.json.gz', setSnippetUsageDownloading)}>{snippetUsageDownloading ? 'Preparing…' : '⇩ Usage diagnostics'}</button></div></div><div className="snippet-usage-summary">{snippetUsage.snippets.map((item) => <article key={item.snippetId || item.title}><div><b>{item.title}</b><small>Snippet #{item.snippetId || 'legacy'} · last used {formatDate(item.lastUsedAt)}</small></div><strong>{item.uses} use{item.uses === 1 ? '' : 's'}</strong><span>{item.queriesAffected} quer{item.queriesAffected === 1 ? 'y' : 'ies'} · {item.users} user{item.users === 1 ? '' : 's'}</span></article>)}{!snippetUsage.snippets.length && <div className="empty-admin">No snippet use has been recorded yet. New question-level traces appear for future uses.</div>}</div></section><section className="settings-card"><div className="settings-head"><div><h2>Snippet-use traces</h2><p>Showing {snippetUsage.events.length} of {snippetUsage.totalEvents} newest events. Open only the trace you need.</p></div></div><div className="snippet-event-list">{snippetUsage.events.map((event) => <details key={event.id}><summary><div><b>{event.title || `Snippet #${event.snippetId}`}</b><small>{event.questionPreview || 'Question text unavailable'}</small></div><span>{event.userName || event.userEmail || 'Unknown user'}<small>{formatDate(event.createdAt)}</small></span></summary><div><p><b>Question</b>{event.question || event.questionPreview || 'Unavailable'}</p><p><b>Scope</b>{event.selectedProduct?.toUpperCase() || 'Not recorded'} · {event.selectedScopeLabel || event.selectedModel || 'Not recorded'}</p><p><b>Correction trigger</b>{event.triggerTerms || 'Not recorded'} · score {event.matchScore ?? 'not recorded'}</p><p><b>Instruction version</b>{event.instruction || 'Unavailable'}{event.instructionHash ? ` (hash ${event.instructionHash})` : ''}</p><p><b>Query trace</b>#{event.queryLogId || 'not recorded'} · {event.injection || 'not recorded'} · influence: {event.influence || 'not recorded'}</p></div></details>)}{!snippetUsage.events.length && <div className="empty-admin">No events recorded yet.</div>}</div>{snippetUsage.nextOffset != null && <div className="row" style={{ marginTop: 12 }}><button className="btn btn-secondary" onClick={() => loadSnippetUsage(snippetUsage.nextOffset, true)}>Load 30 older traces</button></div>}</section></>}</div>}

        {tab === 'reports' && <div className="settings-stack"><section className="settings-card diagnostic-download-card"><div className="settings-head"><div><h2>Combined admin diagnostics</h2><p>One compact compressed download combines the current Notice knowledge, every query-and-answer evaluation record, and all question-level snippet-use traces.</p></div><button className="btn btn-primary compact-download" disabled={diagnosticsDownloading} onClick={() => downloadGzip('/api/reports', 'fundednext-admin-diagnostics.json.gz', setDiagnosticsDownloading)}>{diagnosticsDownloading ? 'Preparing…' : '⇩ Download combined report'}</button></div><div className="diagnostic-download-grid"><div><b>Query evaluation</b><small>All confidence levels, failures, evidence trails, and approved disputes; high-confidence records stay compact.</small></div><div><b>Notice knowledge</b><small>Current and historical Notice records exactly as stored in the knowledge base.</small></div><div><b>Snippet diagnostics</b><small>Usage totals plus each question, scope, instruction version, and query trace.</small></div></div><p className="field-help">The file is gzip-compressed JSON for efficient diagnostics. It contains workspace data, so store it only in approved internal locations.</p></section></div>}
              ...
              return <article id={`query-log-${log.id}`} className={`query-log-card${expanded ? ' expanded' : ''}${selected ? ' selected' : ''}${log.feedback ? ` has-feedback ${log.feedback}` : ''}`} key={log.id}>
                ...
                {expanded && <div className="query-log-detail">
                  ...
                  {(log.interpretation || log.evidenceTrail || log.processing) && <details className="query-audit-details"><summary>Processing and evidence details · {log.evidenceTrail?.selectedForAnswer?.length || 0} selected · {log.evidenceTrail?.rejected?.length || 0} rejected</summary><div className="interpretation-trail">{log.interpretation && <>
                    ...
                    </>}{log.evidenceTrail && <><details className="interpretation-section"><summary>Evidence passed to the answering AI as context ({log.evidenceTrail.selectedForAnswer?.length || 0})</summary><p className="interpretation-empty">This is the filtered context, not a list of claims cited in the final answer. Compare it with “Sources used” above to see what the answer actually cited.</p><div className="evidence-audit-list accepted">{(log.evidenceTrail.selectedForAnswer || []).map((item, index) => <div key={`${item.id}-${index}`}><span>{item.position}</span><div><b>{item.title || item.id}</b><small>{item.kind} · similarity {(Number(item.similarity || 0) * 100).toFixed(1)}%{item.exactScope ? ' · exact Account match' : ''}</small></div>{item.url && <a href={item.url} target="_blank" rel="noreferrer">Open ↗</a>}</div>)}</div></details>
        ...
        {tab === 'autosync' && <div className="settings-stack">
          <section className="settings-card"><div className="settings-head"><div><h2>Automatic FAQ sync</h2><p>Keep the knowledge base current on its own. When on, the app checks Intercom for new or changed articles on the schedule you choose and re-indexes what changed.</p></div><label className="toggle big"><input type="checkbox" checked={!!autoSync?.enabled} onChange={(e) => saveAutoSync({ enabled: e.target.checked }, e.target.checked ? 'Automatic sync turned on.' : 'Automatic sync turned off.')} /><i /></label></div>
            <div className="autosync-grid">
              ...
              <div className="field-block"><label>Last automatic completion</label><b className="autosync-when">{autoSync?.lastAutoSyncAt ? formatDate(autoSync.lastAutoSyncAt) : 'Never run yet'}</b></div>
              <div className="field-block"><label>Last manual completion</label><b className="autosync-when">{autoSync?.lastManualSyncAt ? formatDate(autoSync.lastManualSyncAt) : 'Never run yet'}</b></div>
            ...
            <p className="field-help">Manual and automatic runs use one expiry-based queue lease, so they cannot overwrite or duplicate each other. A partial run remains queued and is resumed by the next scheduled run.</p>
