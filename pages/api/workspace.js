...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      if (action === 'tour') {
        // Registration and guide-state loading are one round trip. Previously
        // this endpoint upserted first and then selected the same row.
        const person = checked(await sb.from('workspace_people')
          .upsert({ email, name: user.name }, { onConflict: 'email' })
          .select('scope_tour_at,answer_tour_at').single());
    ...
    if (req.method === 'POST') {
      if (action === 'tour') {
        ...
        const update = await sb.from('workspace_people').update({ [field]: completedAt }).eq('email', email).is(field, null).select('email');
        if (update.error) throw update.error;
        // A direct POST can arrive before the guide's GET registration call.
        // Only pay for the fallback upsert when the user row truly is absent.
        if (!update.data?.length) {
          const existing = checked(await sb.from('workspace_people').select('email').eq('email', email).maybeSingle());
          if (!existing) checked(await sb.from('workspace_people').upsert({ email, name: user.name, [field]: completedAt }, { onConflict: 'email' }));
        }
        const recorded = await logActivity({ actorRole: user.role, sessionId: user.sessionId, userName: user.name, userEmail: email, authProvider: user.authProvider, eventType: 'workspace_tour_completed', success: true, metadata: { stage: req.body.stage, tourVersion: TOUR_VERSION, completedAt } }, { sb, returnId: false });
        if (!recorded) throw new Error('The tutorial acknowledgement could not be recorded. Please try again.');
