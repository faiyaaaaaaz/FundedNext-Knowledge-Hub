...
function DataLoader({ title = 'Loading workspace', detail = 'Requesting live data…' }) {
  return <div className="real-loader" role="status" aria-live="polite"><span className="loader-mark" aria-hidden="true" /><div><b>{title}</b><small>{detail}</small></div></div>;
}
...
export default function Home() {
  ...
  async function send(question = input.trim(), clarificationAnswer = '', showUser = true) {
    ...
    try {
      ...
      catch { throw new Error(`The server did not finish this request (HTTP ${response.status}). Please try again. The recorded request can be reviewed in Admin query logs.`); }
      ...
      <div className="workspace-grid">
        ...
        <aside className="insights-rail">
          ...
          {statsLoading ? <DataLoader title="Checking knowledge health" detail="Connecting to Supabase and verifying the live index…" /> : <>
            ...
            <div className="metric-card update-time knowledge-time"><span>↻ Knowledge update status</span>{stats?.latestFaq ? <><strong>{stats.latestFaq.title || 'Untitled FAQ'}</strong><small>Latest article indexed: {formatDhaka(stats.latestFaq.last_indexed_at)}</small><small>Article updated: {stats.latestFaq.updated_at ? formatDhaka(typeof stats.latestFaq.updated_at === 'number' ? stats.latestFaq.updated_at * 1000 : stats.latestFaq.updated_at) : 'Not recorded'}</small>{/^https?:\/\//i.test(stats.latestFaq.url || '') && <a href={stats.latestFaq.url} target="_blank" rel="noreferrer">Open FAQ ↗</a>}</> : <small>No completed FAQ sync recorded.</small>}<small>Last automatic update: {stats?.lastSyncAt ? formatDhaka(stats.lastSyncAt) : 'Not recorded'}</small><small>Last manual update: {stats?.lastManualSyncAt ? formatDhaka(stats.lastManualSyncAt) : 'Not recorded'}</small></div>
